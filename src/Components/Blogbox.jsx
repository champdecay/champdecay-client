        ...
        <div className="p-4 lg:w-1/3 sm:w-1/2">
            {featured_image ?
                <div className="h-full border-2 border-gray-200 border-opacity-60 rounded-lg overflow-hidden">
                    ...
                    <div className="p-6">
                        <h4 className="tracking-widest text-xs title-font font-medium text-indigo-300 mb-1">{tags.map(el => `${el} `)} | {dayjs(createdAt).format("Do MMM, YYYY")}</h4>
                        <h2 className="title-font text-lg font-medium text-white mb-3">{name}</h2>
                        <p className="leading-relaxed mb-3 text-white">{excerpt}</p>
                        <div className="flex items-center flex-wrap ">
                            <Link to={slug} className="text-indigo-300 inline-flex items-center md:mb-2 lg:mb-0">Learn More
                ...
                <div className="h-full border-2 border-gray-200 border-opacity-60 px-8 pt-16 pb-24 rounded-lg overflow-hidden text-center relative flex flex-col items-center justify-center">
                    <h4 className="tracking-widest text-xs title-font font-medium text-indigo-300 mb-1">{tags.map(el => `${el} `)} | {dayjs(createdAt).format("Do MMM, YYYY")}</h4>
                    <h2 className="title-font sm:text-2xl text-xl font-medium text-white mb-3">{name}</h2>
                    <p className="leading-relaxed text-white mb-3">{excerpt}</p>
                    <Link to={slug} className="text-indigo-300 inline-flex items-center md:mb-2 lg:mb-0">Learn More
                        <svg className="w-4 h-4 ml-2" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"
                            fill="none" strokeLinecap="round" strokeLinejoin="round">
                            <path d="M5 12h24"></path>
